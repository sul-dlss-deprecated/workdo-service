...
public class FedoraActiveMqTests implements MessageListener{
    ...
    @Test
    public void justSendMessageToDorWorkflowService() throws Exception{
        MessagingService msgService = (MessagingService)appCtx.getBean("dor.messaging");
        msgService.sendObjectUpdatedMessage("dr:12345");
    }
        ...
        while (true) { // Wait for the message
            if (received) {
                ...
                if (currentTime > (startTime + timeout)) {
                    Assert.fail("Timeout reached waiting for message.");
                    break;
                } else {
                    try {
                        Thread.sleep(100);
